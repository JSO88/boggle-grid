...
@app.post("/generate")
def generate(min_length: int = 3, grid_size: int = 4):
    """
    Generate a new Boggle board and return all valid words on it.
    Query params: min_length (default 3), grid_size (default 4)
    Returns: { board: [...], valid_words: [...] }
    """
    grid        = generate_boggle_grid(LETTER_FREQ, size=grid_size)
